...
def load_R52(dataset):
    """
    Load the R52 dataset.
    :param dataset: the dataset dictionary
    """
    load_tab_spaced_data("data/R52/test.txt", "data/R52/train.txt", dataset)
...
def load_20NG(dataset):
    ...
    :param dataset: the dataset dictionary
    """
    load_via_metadata(dataset, "20ng")


def load_ohsumed(dataset):
    """
    Load the ohsumed dataset.
    :param dataset: the dataset dictionary
    """
    load_via_metadata(dataset, "ohsumed")


def load_via_metadata(dataset, name):
    """
    ...
    # The following region was inspired by https://github.com/lgalke/text-clf-baselines/blob/main/data.py#L50
    # region load raw data
    with open(f"data/corpus/{name}.txt", 'rb') as f:
    ...
    with open(f"data/{name}.txt", 'r', encoding='utf-8') as f:
        for idx, line in enumerate(f):
            ...
            if 'test' in train_or_test:
                ...
                test_texts.append(raw_documents[idx])
            elif 'train' in train_or_test:
                train_labels.append(label)
                train_texts.append(raw_documents[idx])
            else:
                raise ValueError("Doc is neither train nor test: in line: " + str(idx + 1))
    # endregion

    label_dict = create_dict(set.union(set(train_labels), set(test_labels)))

    dataset['label_dict'] = label_dict
    dataset['train'] = (train_texts, train_labels)
    dataset['test'] = (test_texts, test_labels)
